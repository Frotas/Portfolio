...
export default function ServicesComponent(props: ServicesProps) {
  const style = {
    section: `
        ...
        shadow-[inset_0_0_5px_5px_rgba(0,0,0,0.4)]        
      `,
    cards: {
      section: `
        w-full
        flex flex-col gap-8
        md:grid md:justify-items-center md:items-center
        md:grid-rows-1 md:grid-cols-2
        md:[&_section:nth-child(3)]:col-span-2
        md:[&_section:nth-child(3)]:max-w-[50%]
      `,
      root: `
        w-[-webkit-fill-available] h-[-webkit-fill-available]
        gap-8 p-8 bg-DarkSlateGray dark:bg-DeepDarkBlue
          ...
          <Root className={style.cards.root}>
            ...
            <Footer className={style.cards.footer}>
              <Paragraph className={style.cards.paragraph}>
                Construção de sistemas robustos e escaláveis no lado do
                servidor, utilizando linguagens como JavaScript e Java,
                juntamente com frameworks como Express e Spring Boot.
              </Paragraph>
              <ToggleList
                title="Serviços"
                tip="Lista Suspensa"
